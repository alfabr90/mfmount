...
static int sf_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi)
{
    ...
    if (tableitem != NULL) {
        ...
        while (nodeitem != NULL) {
            ...
            memcpy(&st, tableitem->node->st, sizeof(struct stat));

            if (filler(buf, tableitem->node->name, &st, 0) != 0) {
                pthread_mutex_unlock(&lock_dir);
...
static int sf_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    if (tableitem != NULL) {
        ret = (int) sf_node_read(buf, size, offset, tableitem->node);
...
static int sf_rename(const char *path, const char *newpath)
{
    ...
    newnodeitem = sf_node_find(newpath);

    pthread_mutex_lock(&lock_tbl);
    tableitem = sf_table_get(nodeitem->ino);
    parenttableitem = sf_table_get(nodeitem->parent->ino);
    newparenttableitem = sf_table_get(newparentnodeitem->ino);

    node = tableitem->node;
    parent = parenttableitem->node;
    newparent = newparenttableitem->node;

    if (newnodeitem != NULL) {
        newtableitem = sf_table_get(newnodeitem->ino);
        newnode = newtableitem->node;
    }
    pthread_mutex_unlock(&lock_tbl);
    ...
    if (S_ISDIR(node->st->st_mode) || S_ISREG(node->st->st_mode)) {
        if (newnodeitem == NULL) {
        ...
        if (S_ISDIR(node->st->st_mode)) {
            if (S_ISDIR(newnode->st->st_mode) || (newnodeitem != NULL && newnodeitem->nodelist != NULL)) {
                pthread_mutex_unlock(&lock_dir);

                pthread_mutex_lock(&lock_tbl);
                sf_table_remove(newnode);
                pthread_mutex_unlock(&lock_tbl);
                ...
                if (S_ISDIR(newnode->st->st_mode)) {
                    ...
                    return -ENOTDIR;
                } else if (newnodeitem != NULL && newnodeitem->nodelist != NULL) {
